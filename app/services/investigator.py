...
class Investigator:
    ...
    async def _run(
        ...
        methodology = [
            ...
            "Оставляем в таблице только сайты, которые удалось открыть и зафиксировать.",
            "Для каждого открытого сайта сохраняем HTML, SHA-256, DNS/TLS и скриншот.",
...
"""
    ...
    async def _build_finding(
        ...
        if not evidence.active or not evidence.final_url or not evidence.html_path or status_code < 200 or status_code >= 400:
            return {
                "_skip": True,
                "_skip_reason": "сайт не открылся с нормальным HTTP 2xx/3xx или не дал HTML для фиксации",
                "url": candidate.url,
                "domain": domain,
                "status_code": evidence.status_code,
            }
